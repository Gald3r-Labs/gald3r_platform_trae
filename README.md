...
<h1 align="center">Gald3r for TRAE (ByteDance)</h1>

<p align="center">
  The <strong>TRAE (ByteDance) edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside TRAE (ByteDance)</strong>. Part of the Gald3r framework
...
This is the **TRAE (ByteDance) distribution** of Gald3r. The [`trae/`](./trae/) folder is a
complete, ready-to-deploy gald3r setup tuned for TRAE (ByteDance) -- drop its contents into your
...
cp -r gald3r_platform_trae/trae/. /path/to/your/project/
```

Then open the project in TRAE (ByteDance) and type `@g-status` to confirm gald3r loaded.

> The `trae/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
